...
/**
 * The class <code>Core</code> is the core class of the system that can be seen
 * as the center amongst all other classes keeping track of all useful
 * informations for all users.
 * 
 * A hashMap of username as key and user as value allows to have an efficient
 * way of activating and deactivating <code>Users</code>.
 * 
 * In order to efficiently display a sorted list of all the <code>Menu</code>
 * and the <code>Dish</code> the <code>TreeMap</code> called
 * <code>MealHeap</code> and <code>DishHeap</code> are implemented. Adding a new
 * order as well as removing one is executed in O(log(n)) with n standing for
...
public class Core implements Serializable {
	...
	/*
	 * Two users can't have the same USERNAME, so we need an HashMap mapping
	 * each USERNAME to its corresponding user.
	 */
	...
	/*********************************************************************/
	/**
	 * This method allows unregistered clients to register themselves. The
	 * method cannot be called if somebody is logged in.
	 * 
	 * @param user
	...
	/**
	 * This method includes password safety and allows registered users to log
	 * in and changes the current user to the user having completed the log in.
	...
	/**
	 * This method allows registered users to log in and changes the current
	 * user to the user having completed the log in. New messages are printed
	 * out if user logs in.
	 * 
	 * @param username
	 *            registered username that is saved in the system.
	...
	/**
	 * Sets the special meal of current_restaurant to the input
	 * <code>Meal</code>. Note that this function has to be here (and not only
	 * in the <code>Restaurant</code> class) because we need to notify all
	 * registered customers after this update.
	 * 
	 * @param meal
	 *            the new special meal of the week
	 */
	...
	/**
	 * Notifies all customers that are active of the new special meal offer. The
	 * fact a user wants to be notified or not is an attribute associated with
	 * each `Customer`.
	 */
	...
	/**
	 * @param rest
	 *            is of type <code>Restaurant</code>
	 * @return mealRestHeap is a TreeSet of all the orders of meals of a
	 *         specific restaurant
	 */
	 ...
	 * @param rest
	 *            is of type <code>Restaurant</code>
	 * @return mealRestHeap is a TreeSet of all the orders of dishes of a
	 *         specific restaurant
	 */
	 ...
	 * 
	 * @param meal
	 *            is of type <code>Meal</code>
	 * @return mCount.getCount() or 0: either the Meal was already ordered then
	 *         its count is returned or the Meal has not been ordered yet then
	 *         its count is 0
	 */
	 ...
	 * This function is exclusively used by the core (=private).
	 * 
	 * This function adds a new meal count each time a meal was ordered and
	 * delivered. It is only used by the function <code>treatNewOrders</code>
	 * once meals have been well ordered.
	 * 
	 * @param meal
	 *            is of type Meal
	 * @param count
	 *            is the amount meal has already been ordered
	 * @param rest
	 *            is of type restaurant
	 */
	 ...
	 * 
	 * @param dish
	 *            is of type <code>Dish</code>
	 * @return dCount.getCount() or 0: either the Dish was already ordered then
	 *         its count is returned or the Dish has not been ordered yet then
	 *         its count is 0
	 */
	 ...
	 * This function is exclusively used by the core (=private).
	 * 
	 * This function adds a new dish count each time a dish was ordered and
	 * delivered. It is only used by the function <code>treatNewOrders</code>
	 ...
	 *            is of type Dish
	 * @param count
	 *            is the amount dish has already been ordered
	 * @param rest
	 *            is of type restaurant
	 */
	...
	/**
	 * @param order
	 *            is either true or false (true = descending order)
	 * @return MealHeap or DishHeap according to the current policy established
	 *         by the managers. returned Heap can be in ascending or descending
	 *         order according to the input
	 */
	 ...
	 *            is either true or false (true = descending order)
	 * @return MealHeap or DishHeap according to the current policy established
	 *         by the managers for the chosen restaurant. returned Heap can be
	 *         in ascending or descending order according to the input
	 */
	...
	/**
	 * This function allows to leave a message for the system. If the current
	 * user is a manager, the message will be displayed directly
	 * 
	 * @param message
	...
	/**
	 * This function allows to leave a message for a user. If the current user
	 * is the destination of the message, the message will be displayed directly
	 * 
	 * @param user
	 *            the user who is going to receive the message
	 * @param message
	...
	/**
	 * Returns the history of orders of the current user, null if none is logged
	 * in.
	 * 
	...
	/**
	 * Depending on the chosen tpPolicy a certain profit related quantity is
	 * calculated:
	 * 
	 * @param profit
	 *            wanted profit
	 * @param input1 
	 *            service fee || markup percentage || delivery cost
	 * @param input2
	 *            markup percentage || delivery cost || service fee
	 * @return delivery cost || service fee || markup percentage (all needed to
	 *         achieve profit)
	 * 
	 *         <ul>
	 ...
	 * 
	 *         </li>
	 *         <li>Case 2: tpPolicy = ServiceFeeProfit
	 * 
	 *         <ul>
	 *         <li>input1 markup percentage</li>
	 ...
	 * 
	 *         </li>
	 *         <li>Case 3: tpPolicy = MarkupProfit
	 * 
	 *         <ul>
	...
	public double simulateProfit(double profit, double input1, double input2) {
		if (current_manager != null) {
			...
			return this.tpPolicy.howToTargetProfit(input1, input2, profit, this.savedOrders, this.dateBefore,
					this.dateAfter);
		} else {
			unauthorizedCommand();
			return 0;
	...
	/**
	 * Returns an ArrayList of restaurants sorted in decreasing (or increasing
	 ...
	 *
	 * @param user
	 *            a User that is currently added to the system
	 * @param username
	 *            new username that the user has
	 */
